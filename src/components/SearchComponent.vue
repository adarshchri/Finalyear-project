<template>
  <div class="w-4/5 max-w-large mt-8">
    ...
    <div class="relative">
      <input
        ...
        class="
          search-input
          bg-tekst
          border border-svetla
          rounded-full
          px-4
          pl-10
          py-2
          outline-none
          focus:border-narandza
          w-full
        "
        ref="search"
        v-model="searchQuery"
    ...
    <transition name="fade">
      <div
        v-if="searchQuery.length > 0 && searchResultsVisible"
        class="
          absolute
          bg-tekst
          border
        ...
        :class="theme === 'theme-dark' ? 'results-dark' : 'results-light'"
      >
        <div class="flex flex-col w-full" ref="results">
          <div
            v-for="(post, index) in searchResults"
            :key="index"
            @click="goToMovie(post.item.id)"
            class="
              flex
              justify-between
              border-b border-svetla
              cursor-pointer
              p-3
              hover:bg-narandzaSvetla
            "
            @mousedown.prevent="searchResultsVisible = true"
            :class="{ 'bg-narandzaSvetla': index === highlightedIndex }"
          >
            <img
              :src="
              ...
              "
              @error="defaultImage"
              class="rounded w-1/6 max-w-smaller h-auto"
            />
            <div class="flex flex-col ml-2 w-96">
              <p
                ...
                "
              >
                {{ post.item.title }}({{ post.item.release_date }})
              </p>
              <p
                class="
                  text-tiny
  ...
  methods: {
    ...
    getMoviesForSearch(search) {
      if (search !== "") {
        axios
          .get(
            "https://api.themoviedb.org/3/search/movie?api_key=3f713b362ad34a78305e2fbd42d24840&language=en-US",
            { params: { query: search } }
          )
          .then((response) => {
